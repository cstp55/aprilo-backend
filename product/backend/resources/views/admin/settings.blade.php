...
@section('content')
    <div class="header">
        <div>
            ...
            <h1 style="display: flex; align-items: center; gap: 12px;">
                @if ($tab === 'agent') Agent Settings
                @elseif ($tab === 'design') Widget Design
                @elseif ($tab === 'connect') Connect Channels
                @elseif (str_starts_with($tab, 'connect_')) Connect Channels ({{ ucfirst(substr($tab, 8)) }})
                @elseif ($tab === 'deploy') Deploy Script
                @elseif ($tab === 'pricing') Plans & Pricing
                @endif
            ...
            <p class="help">
                @if ($tab === 'agent') Configure your assistant's identity, logic configuration, and restrictive AI parameters.
                @elseif ($tab === 'design') Customize your widget themes, colors, and avatar styling.
                @elseif ($tab === 'connect' || str_starts_with($tab, 'connect_')) Stream notifications, connect platforms, and integrate third-party communications.
                @elseif ($tab === 'deploy') Embed script snippets to publish the chatbot live.
                @elseif ($tab === 'pricing') Select billing options and manage subscription plans.
                @endif
            </p>
        </div>
    </div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 340px; gap: 24px; align-items: start;">
        ...
        <!-- Left Column: Settings Config Workspace -->
        <div style="min-width: 0;">
            ...
            @if ($tab === 'agent')
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        const customPromptArea = document.getElementById('custom-prompt-textarea');
                        const strictContext = document.getElementById('strict-context-checkbox');

                        const templates = {
                            strict_retrieval: "You are a strict QA assistant. Answer the user's questions based ONLY on the provided context.",
                            hr_policy: "You are an HR Policy Assistant. Answer questions regarding leaves, benefits, and workplace rules.",
                            general_faq: "You are a helpful general assistant. Answer questions using general corporate rules.",
                            custom: ""
                        };
                        ...
                        templateSelect.addEventListener('change', function() {
                            ...
                            if (val !== 'custom') {
                                customPromptArea.value = templates[val];
                            }
                            if (val === 'strict_retrieval') {
                                strictContext.checked = true;
                            }
                        });
                    });
                </script>
            ...
            <!-- TAB 2: WIDGET DESIGN -->
            @if ($tab === 'design')
                <form class="card" method="POST" action="{{ route('admin.settings.update', ['tab' => 'design']) }}" style="max-width: 600px;">
                    @csrf
                    @method('PATCH')
                    <!-- Maintain other settings using hidden fields -->
                    ...
                    <div class="card" style="position: relative;">
                        @if (!in_array($organization->plan, ['pro', 'enterprise'], true))
                            <div style="position: absolute; inset: 0; background: rgba(255,255,255,0.7); z-index: 10; display: flex; flex-direction: column; align-items: center; justify-content: center; backdrop-filter: blur(2px); border-radius: 8px;">
                                ...
                                <a href="{{ route('admin.settings.pricing') }}" style="margin-top: 10px; font-size: 12px; color: var(--green); text-decoration: underline; font-weight: 700;">Upgrade Plan</a>
                            </div>
                        @endif
            ...
            <!-- TAB 3a: TEAMS CONFIGURATION FORM -->
            @if ($tab === 'connect_teams')
                @include('admin.connect.teams')
            @endif

            <!-- TAB 3b: SKYPE CONFIGURATION FORM -->
            @if ($tab === 'connect_skype')
                @include('admin.connect.skype')
            @endif

            <!-- TAB 3c: WHATSAPP CONFIGURATION FORM -->
            @if ($tab === 'connect_whatsapp')
                @include('admin.connect.whatsapp')
            @endif

            <!-- TAB 3d: SMTP MAIL CONFIGURATION FORM -->
            @if ($tab === 'connect_mail')
                @include('admin.connect.mail')
            @endif
...
></script></textarea>
        ...
        <!-- Right Column: Live Chat Widget Simulator (Shown on all pages) -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Sync widget styling inputs with preview if on the design tab
            ...
            if (widgetIconSelect && previewHeaderIcon) {
            ...
            function sendPreviewMessage() {
                ...
                .then(data => {
                    ...
                    if (data.sources && data.sources.length > 0) {
                        ...
                        const themeColor = colorPicker ? colorPicker.value : '{{ $settings->chatbot_color_palette ?: "#d22630" }}';
